...
public class X86FileWriter {
	...
	public void translate(Expr.Constant e, Register target,
		...
		System.out.println(freeRegisters.contains(target));
		AddrRegRegOp instruction = getInstructionKind(type);
		instructions.add(new Instruction.AddrRegReg(instruction, label, HIP, target));
		freeRegisters.remove(target);
	}

	private AddrRegRegOp getInstructionKind(Type t){
		if(t instanceof Type.Strung){
			return AddrRegRegOp.lea;
		}
		return AddrRegRegOp.mov;
	}
	...
	private void addStringConstant(whilelang.lang.Expr.Constant e, String label, X86File.Data data ) {
		String shit = stripValue(e);
		Constant item = new Constant.String(label, shit);
		data.constants.add(item);
	}

	private void addBooleanConstant(whilelang.lang.Expr.Constant e, String label, X86File.Data data ) {
		int val = (boolean) e.getValue() ? 1 : 0;
		Constant item  = new Constant.Word(label, val);
		data.constants.add(item);
	}

	private String stripValue(Expr.Constant e){
